...
if st.session_state.user is None:
    ...
    with tab2:
        ...
        if st.session_state.get("sem_data"):
            ...
            with c2:
                # 🛠️ WATERMARK LOGIC: Adding branding to Download
                report_text = f"🚀 TOPPERGPT SYLLABUS TRACKER\nProgress: {total_progress}%\n"
                report_text += "-------------------------------------------\n"
                for sem, subs in st.session_state.sem_data.items():
                    report_text += f"\n📌 {sem}\n"
                    for s, ms in subs.items():
                        report_text += f"\n- {s}:\n  " + "\n  ".join([f"[{'X' if m in st.session_state.done_topics else ' '}] {m}" for m in ms])
                report_text += "\n\n-------------------------------------------\n"
                report_text += "🔥 Generated by TopperGPT - Your Engineering Painkiller 💊"
                
                st.download_button("📥 Download Branded Tracker", report_text, file_name="TopperGPT_Roadmap.txt")

            st.divider()
            sem_tab_1, sem_tab_2 = st.tabs(["📘 Semester I", "📗 Semester II"])

            # Render Semester Tabs (Inside the loop as you had it)
            def render_sem(data, prefix):
                for subject, modules in data.items():
                    with st.expander(f"📚 {subject}"):
                        for m in modules:
                            is_checked = st.checkbox(m, key=f"{prefix}_{subject}_{m}", value=(m in st.session_state.done_topics))
            ...
            with sem_tab_1: render_sem(s1_data, "s1")
            with sem_tab_2: render_sem(s2_data, "s2")

            # 📢 VIRAL WHATSAPP LOOP
            st.divider()
            share_msg = f"Bhai, dekh TopperGPT pe mera {total_progress}% syllabus khatam ho gaya! Tu bhi track kar apna Engineering roadmap yahan: [Your_App_Link]"
            whatsapp_url = f"https://wa.me/?text={share_msg.replace(' ', '%20')}"
            
            st.markdown(f"""
                <div style="background-color: #f0f2f6; padding: 20px; border-radius: 15px; text-align: center; border: 2px solid #25D366;">
                    <h4 style="color: #25D366; margin-bottom: 10px;">📢 Flex your Progress!</h4>
                    <p style="color: #555; font-size: 14px;">Share your branded roadmap status with your study group.</p>
