...
def apply_pro_theme():
    st.markdown("""
        ...
        /* Custom Styling for the Google Auth Button generated by library */
        iframe { background-color: white !important; border-radius: 8px !important; }
        </style>
...
# --- 3. OFFICIAL GOOGLE AUTH SETUP (FIXED TYPEERROR) ---
# Yahan individual values pass ki hain taaki list wala error na aaye
...
if not st.session_state.get('connected') and st.session_state.user is None:
    ...
    with col_mid:
        ...
        # OFFICIAL GOOGLE LOGIN POPUP BUTTON
        authenticator.login()
        
        # Sync session state if Google connects
...
with st.sidebar:
    ...
    u_display = st.session_state.user.split('@')[0] if st.session_state.user else "Topper"
    st.markdown(f"**Welcome, {u_display}!**")
    st.divider()
