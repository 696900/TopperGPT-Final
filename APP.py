...
# --- TAB 3: THE FINAL BULLETPROOF EVALUATOR ---
with tab3:
    ...
    # 🔑 Key Retrieval (Direct from Secrets)
    api_key_or = st.secrets.get("OPENROUTER_API_KEY")

    if not api_key_or:
        st.error("Bhai secrets mein key nahi mil rahi! OpenRouter key check karo.")
        st.stop()

    ans_file = st.file_uploader("Upload Your Answer Photo", type=["jpg", "png", "jpeg"], key="final_boss_v999")
    ...
    if ans_file:
        ...
        if st.button("🚀 Evaluate Now (Priority: Claude 3.5)"):
            if st.session_state.user_data['credits'] >= 5:
                with st.spinner("Moderator (Claude 3.5 Sonnet) is marking your paper..."):
                    try:
                        # Universal Base64 Encoding
                        buf = io.BytesIO()
                        img_raw.save(buf, format="JPEG")
                        b64_data = base64.b64encode(buf.getvalue()).decode('utf-8')

                        # Direct OpenRouter Request (Stable & High Quota)
                        ...
                        payload_or = {
                            ...
                            "messages": [{
                                "role": "user",
                                "content": [
                                    {"type": "text", "text": "Extract Question & Answer. Evaluate marks out of 10 for engineering accuracy. Return ONLY JSON: {\"question\": \"...\", \"answer\": \"...\", \"marks\": 8, \"feedback\": \"...\"}"},
                                    {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64_data}"}}
                                ]
                            }]
                        }

                        res_or = requests.post(url_or, headers=headers_or, json=payload_or, timeout=45)
                        
                        if res_or.status_code == 200:
                            ai_output = res_or.json()['choices'][0]['message']['content']
                            eval_res = get_clean_json_v2(ai_output)
                            ...
                            st.error(f"Claude Error: {res_or.status_code}")
                            st.json(res_or.json())
