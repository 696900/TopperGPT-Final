...
export function AiTutor() {
  ...
  const tokenEstimate = useMemo(() => {
    const chars = messages.reduce((sum, m) => sum + textOf(m).length, 0)
      ...
      <aside className="hidden min-h-0 flex-col gap-3 xl:flex">
        <div className="glass rounded-2xl p-4">
          ...
          <div className="space-y-2.5">
            <Stat
              icon={<Zap className="size-4 text-academic" />}
              label="Tokens used"
          ...
          <div className="mt-3">
            <div className="mb-1 flex justify-between text-[11px] text-muted-foreground">
              ...
              <span>{Math.min(100, Math.round((tokenEstimate / 8000) * 100))}%</span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-secondary">
              <div
                className="h-full rounded-full bg-gradient-to-r from-cyber to-cyan-neon transition-all"
                style={{
                  width: `${Math.min(100, (tokenEstimate / 8000) * 100)}%`,
                }}
              />
            </div>
          </div>
