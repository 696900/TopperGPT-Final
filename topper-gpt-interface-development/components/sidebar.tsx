...
import { ChevronLeft, PanelLeftOpen, Zap, X } from 'lucide-react'
    ...
    <>
      ...
      >
        <nav className="glass flex h-full flex-col rounded-2xl p-3">
          ...
          {/* Upgrade card */}
          {!collapsed && (
            <div className="mt-3 rounded-xl border border-cyber/30 bg-gradient-to-br from-cyber/15 to-cyan-neon/5 p-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-white">
                <Zap className="size-4 text-academic" />
                TopperGPT Pro
              </div>
              <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">
                Unlimited predictions, priority tutor, and export tools.
              </p>
              <button className="mt-2.5 w-full rounded-lg bg-gradient-to-r from-cyber to-cyan-neon py-1.5 text-xs font-semibold text-white transition-opacity hover:opacity-90">
                Upgrade
              </button>
            </div>
          )}
