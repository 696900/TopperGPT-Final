...
import { Menu, MessageSquarePlus, Search, Sparkles } from 'lucide-react'
...
export function Topbar({
  ...
  return (
    <header className="glass sticky top-0 z-30 flex items-center gap-3 rounded-2xl px-4 py-3">
      ...
      <div className="flex items-center gap-2 rounded-full border border-cyber/30 bg-cyber/10 py-1 pl-1 pr-3">
        <div className="grid size-7 place-items-center rounded-full bg-gradient-to-br from-cyber to-cyan-neon text-xs font-bold text-white">
          AS
        </div>
        <div className="hidden text-left leading-tight sm:block">
          <div className="flex items-center gap-1 text-xs font-semibold text-white">
            Pro
            <Sparkles className="size-3 text-academic" />
          </div>
        </div>
      </div>
    </header>
